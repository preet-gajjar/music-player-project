...
def get_album_cover():
    ...
    image_current_path=f'/img/{image_current_path}.jpg'
...
def remove_album_cover():
    image_current_path=f'/img/Transparent.png'
    image1 = Image.open(image_current_path)
    image2=image1.resize((170, 170))
    load = ImageTk.PhotoImage(image2)
    label1.configure(image=load)
...
#create master frame
master_frame = customtkinter.CTkFrame(root, width=920, height=450)
master_frame.pack(pady=20)

#load and create background imag
bg_image_label = tkinter.Label(master_frame)
current_path = os.path.dirname(os.path.realpath(__file__))
image1 = Image.open(current_path + "/Images/Background1.jpg")
# image2=image1.size(920,450)
load = ImageTk.PhotoImage(image1)
bg_image_label.configure(image=load)
bg_image_label.image=load
